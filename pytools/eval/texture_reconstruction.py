...
class TextureReconstructionInference(Inference, InferenceOptions):
    ...
    def log_images(
        ...
        if self.logging_options.fft_rad:
            log_grid_plt(
                radial_profile(_x.mean(dim=1), device=self.device),
                radial_profile(
                    *map(partial(torch.mean, dim=1), _ys), 
                    device=self.device
                ),
    ...
    def parse_and_log_images(self) :
        ...
        if nfiles==1:
            ...
            for i in range(nfiles):
                start = self.logging_options.nimg * i
                stop  = self.logging_options.nimg * (i + 1)
                self.log_images(
                    *slice_tensors(x, start=start, stop=stop),
                    *slice_tensors(y, start=start, stop=stop),
                    idx=i+1
                ) 

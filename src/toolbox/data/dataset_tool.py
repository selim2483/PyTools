...
from ..utils.path import file_ext, is_image_ext
...
def open_lmdb(lmdb_dir: str, *, max_images: Optional[int]):
    ...
    with lmdb.open(
        lmdb_dir, readonly=True, lock=False).begin(write=False) as txn:
...
def open_mnist(images_gz: str, *, max_images: Optional[int]):
    ...
    images = np.pad(
        images, [(0,0), (2,2), (2,2)], 'constant', constant_values=0)
...
def make_transform(
    ...
    def scale(width:int, height:int, img:np.ndarray):
        w = img.shape[1]
        h = img.shape[0]
    ...
    def center_crop(width:int, height:int, img:np.ndarray):
    ...
    def center_crop_wide(width:int, height:int, img:np.ndarray):
    ...
    if transform == 'center-crop':
        if (output_width is None) or (output_height is None):
            error(
                'must specify --resolution=WxH when using ' 
                + transform 
                + 'transform')
    ...
    if transform == 'center-crop-wide':
        ...
        return functools.partial(
            center_crop_wide, output_width, output_height)
...
def open_dest(dest: str) -> Tuple[
    str, Callable[[str, Union[bytes, str]], None], Callable[[], None]]:
    ...
    if dest_ext == 'zip':
        ...
        zf = zipfile.ZipFile(
            file=dest, mode='w', compression=zipfile.ZIP_STORED)
...
def convert_dataset(
    ...
    --source dataset.zip                Recursively load all images from 
                                        dataset.zip
    ...
    zip archive.
    Zip archives makes it easier to move datasets around file servers and
    clusters, and may offer better training performance on network file
    systems.
    ...
    Output images must be square-shaped and they must all have the same
    ...
    used with the --resolution option.
    For example:
    ...
    labels = []
    for idx, image in track(
        enumerate(input_iter), description="Saving images",total=num_files):
        idx_str = f'{idx:08d}'
